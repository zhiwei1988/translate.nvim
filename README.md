...
require('translate').setup({
  ...
  -- 悬浮译文窗
  float = {
    -- 两者同一套规则：≤ 1 为比例（1 即全部），> 1 为绝对行/列数，
    -- false 或非正数 = 不设上限。注意 1.0 与 1 在 LuaJIT 里无法区分，
    -- 故 1 一律读作「全部」而非「1 行/列」。
    max_height = 0.5,           -- 比例相对【编辑器高度】
    max_width  = 0.6,           -- 比例相对【源窗口宽度】
                                -- 正文写满窗口时，"宽度跟随段落"就等于占满
                                -- 窗口，所以默认收窄；不想收窄就设 false
